...
double birth_variance = 0.01;
double death_variance = 0.01;
int wall = 0;
...
public:
    ...
    :population(Init_population), discretization(Discretization), width(Width) {
        ...
        cells = std::vector<Cell>(discretization);
        birth_kernel = ceil(max_distance / grid.get_cell_size());
        death_kernel = ceil(max_distance / grid.get_cell_size());
        // empty cells initialization
        ...
        // computing kernel on the distance lower than 3 variances
        for (long i = 0; i != birth_kernel.size(); ++i) {
